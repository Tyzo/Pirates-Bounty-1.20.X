...
public class MusketItem extends Item {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		world.playSound(null, user.getX(), user.getY(), user.getZ(), ModSounds.SHOOT_1, SoundCategory.NEUTRAL, 1.5F, 1F);
		user.getItemCooldownManager().set(this, 40);

		if (!world.isClient()) {
			MusketBallEntity musketProjectile = new MusketBallEntity(world, user);
			musketProjectile.setVelocity(user, user.getPitch(), user.getYaw(), 0.0F, 0.25F, 0.0F);
